		...
		success : function(data){
			...
			for(var i = 0 ; i < result.length ; i++){
				...
				str += "<td><img class='img-rounded' style='width: 100px; height: 100px;' src='img/"+ result[i].filename+"'></td>" + 
							"<td class='text-center' id='pname'>"+ result[i].pname +"</td><td class='text-center' id='price'>"+ result[i].price + "원"+"</td>" +
				             "<td class='text-center' id='product_count'>" + result[i].product_count + "개"+"</td><td class='text-center' id='mile_give'>" + (result[i].product_count * result[i].price)*0.01 + "원"+"</td>" +
				             "<td class='text-center' id='del_price'>" + fee + "원"+"</td>" + "<td class='text-center'>" + result[i].product_count * result[i].price + "원"+"</td>";
				str += "</tr>";
			}
				str+=	"<tr class='bg-light'>";
				str+=	"<td colspan='8' class='text-right'>상품구매금액 : " +productsum +"원" + "+" + "배송비 :"+fee +"원  =  합계 :" +allsum +"원</td></tr>"
				
			$("#records").html(str);
			$("#psum").html(productsum);
			$("#fee").html(fee);
			$("#totalprice").html(allsum);
			$("#fee2").html(fee);
			$("#totalprice2").html(allsum);
			$("#mile").html(mile);
...
function f_save(){
	...
	if($("input:radio[name='pay']").is(":checked") == true){
	 ...
	 /* 마일리지 유효성 체크 */
	var mile = Number($("#mile").text())
	var total = Number($("#totalprice2").text())
	
	 if($("#mile_used").val() < 1000 && $("#mile_used").val() > 1){
		 alert("최소 사용 금액은 1000원이상입니다.");
		 return false;
   ...
   <div class="col align-self-start">
    ...
    <div  class="ml-5 mt-3">
         ...
         <input type="radio" value="무통장" name="pay" id="pay"> 무통장 &nbsp;&nbsp;&nbsp;
          <input type="radio" value="카카오페이" name="pay" id="pay"> 카카오페이</div>
